...
| Check or manage a Cloud Nexus Endpoint's caller-Namespace allowlist; understand or raise the 1,000-caller default | Nexus caller limits | [nexus-caller-limits.md](references/ops/nexus-caller-limits.md) |
...
- [nexus-caller-limits.md](references/ops/nexus-caller-limits.md) — Cloud-only: the 1,000-caller-Namespace-per-Nexus-Endpoint Access-Policy default; inspecting the current allowlist; adding, removing, replacing entries via `tcld nexus endpoint allowed-namespace`; seeding at create via `--allow-namespace`; raising the ceiling via support ticket.
