...
| Serverless Worker (AWS Lambda / GCP Cloud Run) stopped processing work after a Namespace failover | Serverless HA | Confirm a `FailoverNamespace` audit event, then check whether the Worker Deployment Version's compute provider is in the new active region | [serverless-ha.md#diagnostic-signal--did-a-failover-happen](references/triage/serverless-ha.md#diagnostic-signal--did-a-failover-happen) |
