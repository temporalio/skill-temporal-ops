...
| Ship Temporal Cloud OpenMetrics to SigNoz (OTel Collector + dashboard) | Cloud metrics | [cloud-metrics-signoz.md](references/ops/cloud-metrics-signoz.md) |
...
- [cloud-metrics-signoz.md](references/ops/cloud-metrics-signoz.md) — Ship Temporal Cloud OpenMetrics to SigNoz via an OpenTelemetry Collector: Prometheus receiver against `metrics.temporal.io` with Bearer auth, OTLP exporter to SigNoz, dashboard import, scrape/cardinality/rate-limit awareness, common misconfigurations, hand-off to SigNoz-owned template.
