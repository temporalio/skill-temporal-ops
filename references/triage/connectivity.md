...
**Symptom shape:** OS-level `connection refused` reported inside a gRPC `UNAVAILABLE` status <!-- grpc: UNAVAILABLE -->, or by tools like `nc` / `curl` before any gRPC is attempted. The string `connection refused` is emitted by the Go `net` package wrapping `ECONNREFUSED` <!-- go: net -->.
...
On macOS/BSD `nc`, `-z` scans without sending data, `-v` is verbose, `-w <timeout>` sets the idle/connection timeout in seconds <!-- man: nc(1) -->. If this fails with "Connection refused" no higher layer (TLS, gRPC, auth) can succeed.

## DNS

**Symptom shape:** errors containing `no such host` (Go's `net` package wrapping `EAI_NONAME` / NXDOMAIN) <!-- go: net -->, or resolver-specific messages from the libc resolver (`nodename nor servname provided`, `Temporary failure in name resolution`) bubbled up through Go.
...
```bash
nslookup <namespace>.<account>.tmprl.cloud   # man: nslookup(1) — BIND9
dig <namespace>.<account>.tmprl.cloud        # man: dig(1) — BIND9
...
**First check:**
- Run `nc -zvw10 <host> 7233` from the failing environment. A connect with no data is enough to confirm TCP reachability; that isolates the problem above layer 2. <!-- man: nc(1) -->
...
Temporal Cloud supports private connectivity via AWS PrivateLink and GCP Private Service Connect in addition to the default public internet endpoints <!-- docs/cloud/connectivity/index.mdx:24-28 -->. This section is layer 1–2 diagnosis only; the full setup reference is in `docs/cloud/connectivity/`.
...
|---|---|---|---|
| `no such host` <!-- go: net --> | Go net / libc resolver | 1 (DNS) | Endpoint hostname typo; missing `.<account>` suffix; resolver / VPN / private DNS configuration |
| `connection refused` <!-- go: net --> | Go net (ECONNREFUSED) | 2 (TCP) | Wrong port, dev server not running, data-plane command pointed at `saas-api.tmprl.cloud` <!-- docs/cloud/operation-api.mdx:135 --> |
| `i/o timeout` <!-- go: net --> | Go net | 2 (TCP) | Silent drop by firewall / NACL / NetworkPolicy |
| `network is unreachable` <!-- go: net --> | Go net (ENETUNREACH) | 2 (routing) | No route from client subnet to target |
| `UNAVAILABLE` <!-- grpc: UNAVAILABLE --> with a connection-layer cause | gRPC status | 2–3 | Peel back to the wrapped `net` error; `UNAVAILABLE` on its own can also come from TLS and gRPC layers |
| `context deadline exceeded` <!-- go: context --> | Go context | ambiguous | Not diagnostic on its own; see [runtime-errors.md](runtime-errors.md) |
| `Failed reaching server: last connection error` | Temporal Cloud client diagnostics | usually 3 (TLS) | Most often an expired TLS cert per the troubleshooting guide <!-- docs/troubleshooting/last-connection-error.mdx:15 --> — jump to [certificates.md](certificates.md) |
